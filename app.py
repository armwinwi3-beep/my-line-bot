...
def api_data():
    ...
    try:
        ...
        # 🌟 1. อ่านรายชื่อชีตทั้งหมด
        worksheets = sh.worksheets()
        
        # 🌟 2. ดึงข้อมูลทีเดียวรวบยอด (A2:J) ประหยัดเวลาโหลดไปได้มหาศาล!
        ranges = [f"'{ws.title}'!A2:J1000" for ws in worksheets]
        try:
            batch_res = sh.values_batch_get(ranges)
            value_ranges = batch_res.get('valueRanges', [])
        except Exception as e:
            print("Batch Get Error:", e)
            value_ranges = []
...
def handle_text_message(event):
    ...
    try:
                ...
                for row in rows[1:]:
                    if len(row) >= 4:
                        try:
                            ...
                            if record_type == "รายรับ": 
                                income_accounts_all[account] = income_accounts_all.get(account, 0) + amt
                            elif record_type == "รายจ่าย" or record_type == "ให้ยืมเงิน": 
                                expense_accounts_all[account] = expense_accounts_all.get(account, 0) + amt
                            elif record_type == "ได้คืนจากลูกหนี้":
                                income_accounts_all[account] = income_accounts_all.get(account, 0) + amt
                            elif record_type == "รายจ่ายต้องชำระต่อเดือน":
                            ...
                            if is_target_month:
                                if record_type == "รายรับ" or record_type == "ได้คืนจากลูกหนี้": 
                                    total_income += amt
        ...
        if is_number:
            ...
            col_values = worksheet.col_values(1)
            next_row = len(col_values) + 1
            worksheet.insert_row([date_str, time_str, "รอระบุประเภท", amount, "รอระบุบัญชี", "รอระบุหมวดหมู่", "-", "-", "รอระบุสถานะบิล", "-"], index=next_row)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get('success'):
                ...
                col_values = worksheet.col_values(1)
                next_row = len(col_values) + 1
                worksheet.insert_row([date_str, time_str, "รายจ่าย", amount, "รอระบุบัญชี", "รอระบุหมวดหมู่", sender, receiver, "-", "-"], index=next_row)
